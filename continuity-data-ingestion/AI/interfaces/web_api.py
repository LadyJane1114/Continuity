...
from utils.segment_manager import SegmentManager
...
class SegmentIngestionRequest(BaseModel):
    """Request model for story segment ingestion."""
    text: str
    user_id: str
...
def create_app(
    ...
    segment_manager = SegmentManager(vector_db, entity_extractor, entity_store)
    ...
    @app.post("/ingest/story-segment")
    async def ingest_story_segment(request: SegmentIngestionRequest):
        """Ingest a story segment into the vector database."""
        try:
            logger.info(f"Ingesting segment for user {request.user_id} (length: {len(request.text)})")
            
            result = await segment_manager.ingest_segment(
                text=request.text,
                user_id=request.user_id,
            ...
            logger.error(f"Failed to ingest segment: {e}", exc_info=True)
            raise HTTPException(status_code=500, detail=str(e))
    ...
    async def get_db_stats():
        ...
        try:
            return {
                "vector_db": vector_db.get_collection_info(),
                "entity_store": entity_store.get_stats(),
